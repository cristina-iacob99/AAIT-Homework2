...
image_folder_path = './data/task2/'
label_folder_path = './data/task2/task2/train_data/annotations.csv'

image_data = []

# Read the labels from the CSV file
label_df = pd.read_csv(label_folder_path)
label_dict = label_df['label_idx']
labels = label_dict.tolist()
# Iterate through the list of image file names
for image_filename in label_df['renamed_path']:
    # Open the image
...
with tf.device('/device:GPU:0'):
  ...
  for train, test in kfold.split(image_data, labels):
      ...
      # save the figure
      plt.savefig('./figs/Task2_training1.png')
      plt.close()
      ...
      # save the figure
      plt.savefig('./figs/Task2_training2.png')
      plt.close()
      # Append the trained model to the list
      models.append(model)

  model.save('models/ResNetKFold_Task2.keras')

# Set the directory path where the validation images are located
val_folder_path = './data/task2/task2/val_data/'
...
classification_df.to_csv('validation_task2.csv', index=False)